...
//   client side filtering
    const users =allUsers?.filter((u) => {
        ...
        return u.name?.toLocaleLowerCase().includes(query) || u.email?.toLocaleLowerCase().includes(query)
    })
  ...
  return (
    <SafeAreaView className='flex-1 bg-black' edges={["top"]}>
      <View className='flex-1 bg-black/40 justify-end'>
        <View className='bg-surface rounded-3xl h-[95%] overflow-hidden'>
            <View className='px-5 pt-3 pb-3 bg-surface border-b border-surface-light flex-row items-center'>
                <Pressable className='w-9 h-9 rounded-full items-center justify-center mr-2 bg-surface-card' onPress={() => router.back()}>
